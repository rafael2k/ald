 ...
 *
 * $Id: c_enable.c,v 1.1.1.1 2004/04/26 00:40:51 pa33 Exp $
 */
...
int
c_enable(struct aldWorkspace *ws, int ac, char **av)

{
  ...
  if (ac < 2)
  {
    Print(ws, P_COMMAND, "Syntax: enable <number | all>");
    return (0);
  }
  ...
  if (num)
  {
    ...
    if (!ptr)
      ...
      activateBreakpoint(ws->debugWorkspace_p, ptr);
  }
  else
    activateBreakpoint(ws->debugWorkspace_p, 0);
